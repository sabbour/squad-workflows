...
export async function runInit(repoRoot, { token, owner, repo, force }) {
  ...
  // 8. Install pr-feedback-loop skill
  const skillDir = join(repoRoot, '.copilot', 'skills', 'pr-feedback-loop');
  const skillPath = join(skillDir, 'SKILL.md');
  if (!existsSync(skillPath) || force) {
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(skillPath, buildPrFeedbackLoopSkillContent());
    results.instructions.push('.copilot/skills/pr-feedback-loop/SKILL.md installed');
  } else {
    results.instructions.push('.copilot/skills/pr-feedback-loop/SKILL.md exists (use force to overwrite)');
  }

  return results;
}

export function buildPrFeedbackLoopSkillContent() {
  return `---
